...
Tìm tham số tốt nhất cho CCV bằng cách đánh giá trên tập validation (20% từ train)
"""
...
def grid_search_ccv():
    """
    Grid search để tìm tham số tốt nhất cho CCV
    """
    print("=" * 80)
    ...
    for target_size, n_bins, threshold, color_space in product(
        ...
        print(f"\n{'='*80}")
        print(f"Feature params: target_size={target_size}, n_bins={n_bins}, threshold={threshold}, color_space={color_space}")
        print(f"{'='*80}")
    ...
    if len(results) > 0:
        ...
        for i, result in enumerate(results_sorted[:10], 1):
            ...
            print(f"   Feature: target_size={result['target_size']}, n_bins={result['n_bins']}, "
