# -*- coding: utf-8 -*-
"""
Phân tích kết quả Grid Search cho Histogram Features
...
def analyze_grid_search_results(result_file):
    ...
    """
    print("=" * 100)
    print("PHÂN TÍCH KẾT QUẢ GRID SEARCH - HISTOGRAM FEATURES")
    print("=" * 100)
    ...
    svm_to_histogram = defaultdict(list)
    ...
    for result in results:
        ...
        svm_to_histogram[svm_key].append(histogram_params)
    ...
    for svm_key in svm_params_list:
        histogram_list = svm_to_histogram[svm_key]
        best_hist = max(histogram_list, key=lambda x: x['val_accuracy'])
        
        svm_best_results.append({
            'kernel': svm_key[0],
            'C': svm_key[1],
            'gamma': svm_key[2],
            'best_val_acc': best_hist['val_accuracy'],
            'best_histogram': best_hist
        })
    ...
    for i, result in enumerate(svm_best_results_sorted, 1):
        ...
        print(f"    Best Histogram: target_size={result['best_histogram']['target_size']}, "
              f"color_space={result['best_histogram']['color_space']}, "
              f"bins={result['best_histogram']['bins']}")
    
    return svm_best_results_sorted
def train_and_evaluate_svm_config(config, config_idx, total_configs, results_base_dir='histogram_results'):
    """
    Huấn luyện và đánh giá một cấu hình SVM trên tập test
    
    Args:
        config: Dictionary chứa thông tin cấu hình SVM và histogram
    ...
    C = config['C']
    gamma = config['gamma']
    hist_params = config['best_histogram']
    
    target_size = hist_params['target_size']
    color_space = hist_params['color_space']
    ...
    try:
        ...
        X_train, y_train, class_names = extract_histogram_from_dataset(
            'vn-signs/train',
        ...
        X_test, y_test, _ = extract_histogram_from_dataset(
            'vn-signs/test',
            target_size=target_size,
            color_space=color_space,
            bins=bins,
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            f.write(f"Histogram Parameters:\n")
...
def main():
    ...
    result_file = 'grid_search_results/histogram_grid_search_20251123_084311.pkl'
    ...
    # ========================================================================
    # BƯỚC 2: TẠO THỦ MỤC HISTOGRAM_RESULTS
    # ========================================================================
    print("\n" + "=" * 100)
    print("TẠO THỦ MỤC HISTOGRAM_RESULTS")
    print("=" * 100)
    
    results_base_dir = 'histogram_results'
