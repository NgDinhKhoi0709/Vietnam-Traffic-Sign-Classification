# -*- coding: utf-8 -*-
"""
Phân tích kết quả Grid Search cho HOG Features
...
def analyze_grid_search_results(result_file):
    ...
    """
    print("=" * 100)
    print("PHÂN TÍCH KẾT QUẢ GRID SEARCH - HOG FEATURES")
    print("=" * 100)
    ...
    for result in results:
        ...
        hog_params = {
            ...
            'pixels_per_cell': result['pixels_per_cell'],
            'cells_per_block': result['cells_per_block'],
        ...
        svm_to_hog[svm_key].append(hog_params)
    ...
    for svm_key in svm_params_list:
        hog_list = svm_to_hog[svm_key]
        best_hog = max(hog_list, key=lambda x: x['val_accuracy'])
        
        svm_best_results.append({
            'kernel': svm_key[0],
            'C': svm_key[1],
            'gamma': svm_key[2],
            'best_val_acc': best_hog['val_accuracy'],
            'best_hog': best_hog
        })
    ...
    for i, result in enumerate(svm_best_results_sorted, 1):
        print(f"\n{i:2d}. SVM: kernel={result['kernel']:8s} | C={result['C']:6.1f} | gamma={result['gamma']}")
        print(f"    Best Val Acc: {result['best_val_acc']*100:.2f}%")
        print(f"    Best HOG: target_size={result['best_hog']['target_size']}, "
              f"orientations={result['best_hog']['orientations']}, "
              f"pixels_per_cell={result['best_hog']['pixels_per_cell']}, "
              f"cells_per_block={result['best_hog']['cells_per_block']}")
    
    return svm_best_results_sorted


def train_and_evaluate_svm_config(config, config_idx, total_configs, results_base_dir='hog_results'):
    """
    Huấn luyện và đánh giá một cấu hình SVM trên tập test
    
    Args:
        config: Dictionary chứa thông tin cấu hình SVM và HOG
    ...
    C = config['C']
    gamma = config['gamma']
    hog_params = config['best_hog']
    
    target_size = hog_params['target_size']
    ...
    if kernel == 'linear':
        ...
        folder_name = f"svm_kernel-{kernel}_C-{C}_gamma-{gamma}"
    ...
    try:
        ...
        X_train, y_train, class_names = extract_hog_from_dataset(
            'vn-signs/train',
        ...
        X_test, y_test, _ = extract_hog_from_dataset(
            'vn-signs/test',
            target_size=target_size,
            orientations=orientations,
            pixels_per_cell=pixels_per_cell,
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            f.write(f"  orientations:     {orientations}\n")
            f.write(f"  pixels_per_cell:  {pixels_per_cell}\n")
            f.write(f"  cells_per_block:  {cells_per_block}\n\n")
            f.write(f"Results:\n")
...
def main():
    ...
    result_file = 'grid_search_results/hog_grid_search_20251123_161607.pkl'
    ...
    # ========================================================================
    # BƯỚC 2: TẠO THỦ MỤC HOG_RESULTS
    # ========================================================================
    print("\n" + "=" * 100)
    print("TẠO THỦ MỤC HOG_RESULTS")
    print("=" * 100)
    
    results_base_dir = 'hog_results'
